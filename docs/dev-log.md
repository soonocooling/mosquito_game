...
| 12 | **부기 필터 진하게** (팀장 요청): 세기 0.25→0.45(상한 0.6→0.8), 크기 0.12→0.16 faceW, 더 붉은 색 | #15 |
| 13 | **F-10 분열** (jayden000h): 잡힌 자리에서 반대 방향으로 두 마리, 0.6 s 무적 | #16 |
| 14 | **모기 수 상한 제거** (팀장 결정) + 분열 시 절반 평생 무적을 실제로 연결 (#14 기능이 #16에서 호출되지 않고 있었음) | #17 |
| 15 | **평생 무적 제거** (팀장 결정: 잘 죽는 게 낫다, 어차피 분열함) + **모기가 화면 밖으로 못 나감** (가장자리 0.12 faceW 안쪽) | #18 |
...
- **품질 단계(F-15)**: 스펙의 3단계 "glScale 0.75 → 0.5"를 두 단계로 나누고, 모기 상한 단계는 없애서 총 7단계. 7단계(손 인식 끔)에서는 복귀하지 않는다.
- **모기 수 상한 없음** (팀장 결정, 스펙 결정 5 변경): 모기는 분열로 무제한 늘어난다. 느려지면 F-15가 손 인식 간격·해상도·부기 수를 먼저 낮춘다.
- ~~분열 모기 절반 평생 무적~~ (#14, #17) → **#18에서 제거**. 잘 죽어도 어차피 분열로 늘어나므로 무적은 두지 않는다 (분열 직후 0.6 s 무적만 유지).
- **모기는 화면 안에만**: 가장자리에서 스프라이트 반폭(0.12 faceW) 안쪽에 가두고, 바깥으로 향하던 속도는 0.
- **부기 필터**: 스펙 시작값보다 진하게 조정 (`src/render/config.ts`).
...
├─ game/                모기(F-04, F-06), 잡기(F-08), 피 튀김(F-09), 분열(F-10), Game 구현(index.ts)
